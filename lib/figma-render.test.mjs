...
test('RENDER_TEMPLATE_JS usa setCurrentPageAsync (regra 9 figma-use; sync setter throws)', () => {
...
test('buildRenderJs com manifest sem nodes joga RenderJsError code=INVALID_MANIFEST', () => {
  let caught
...
test('buildRenderJs com feature manifest sem flows joga RenderJsError', () => {
  const m = { type: 'feature', nodes: [{ id: 'context' }] }
  let caught
  try { buildRenderJs({ manifest: m, selectionIds: [] }) } catch (e) { caught = e }
  assert.ok(caught instanceof RenderJsError)
  assert.equal(caught.code, 'INVALID_MANIFEST')
})

test('buildRenderJs replace é global (caso template tivesse 2 ocorrências, todas seriam trocadas)', () => {
  // Indireto: cria manifest válido, executa, confirma que o output não tem
  // NENHUMA ocorrência das strings placeholder.
  const m = makeManifest('feature', { problemStatement: 'X', flows: [{ name: 'F', screens: 1 }] })
  const js = buildRenderJs({ manifest: m, selectionIds: ['x'] })
  assert.equal(js.indexOf('__MANIFEST_JSON__'), -1)
  assert.equal(js.indexOf('__SELECTION_JSON__'), -1)
})
