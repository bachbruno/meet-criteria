...
test('render JS creates screen-justification text node per slot', () => {
  ...
  // Confirm the helper body actually calls setPluginData(t, spec.pluginData) — not just that the role appears in the manifest JSON.
  assert.match(js, /buildScreenJustificationText[\s\S]*?setPluginData\(t,\s*spec\.pluginData\)/)
})
...
test('render JS applies analysis-section pluginData per sub-section', () => {
  ...
  // Confirm the AnalysisOverview loop calls setPluginData(sub, sec.pluginData) — not just that the role appears in the manifest JSON.
  assert.match(js, /buildAnalysisOverviewCard[\s\S]*?setPluginData\(sub,\s*sec\.pluginData\)/)
})
