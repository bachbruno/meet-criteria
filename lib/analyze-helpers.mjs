...
// the node IDs to shift. buildListSlotsJs (Task 11 + this fix) populates
// flowChildIds via plugin-data scan, so multi-flow legacy migrations
// correctly shift downstream rows.
...
const childIdsByFlow = new Map() // flowId -> [nodeId, ...]

function visit(node) {
  const fid = meta(node, 'flowId')
  if (fid) {
    if (!childIdsByFlow.has(fid)) childIdsByFlow.set(fid, [])
    childIdsByFlow.get(fid).push(node.id)
  }
  const r = role(node)
  if (r === 'screen-tag') {
    const fid = meta(node, 'flowId')
...
return JSON.stringify({
  ...
  sideCardIds,
  flowChildIds: Object.fromEntries(childIdsByFlow),
