...
import { screenBlockHeight } from './layout-feature.mjs'
...
test('feature: produces section with computed dimensions and node list', () => {
  ...
  const blockH = screenBlockHeight()
  assert.equal(m.section.height, 2 * C.SECTION_PADDING + blockH)
...
test('feature: analysis-overview has 5 sections with English headings', () => {
  const m = buildRenderManifest(baseArgs('feature', {
  ...
  assert.deepEqual(ao.sections.map((s) => s.key), ['resolution', 'validation', 'attention', 'discussion', 'gap-check'])
})
...
test('AnalysisOverview does not duplicate gap-check when template already lists it', () => {
  const template = loadTemplate('feature')
  // Hand-mutate a clone to simulate post-Task-5 state where templates/feature.jsonc lists gap-check.
  const patched = {
    ...template,
    structure: template.structure.map((n) =>
      n.component === 'AnalysisOverview'
        ? { ...n, sections: [...(n.sections ?? []), 'gap-check'] }
        : n,
    ),
  }
  const identity = resolveIdentity({ mode: 'default' })
  const manifest = buildRenderManifest({
    template: patched, identity,
    slug: 'foo-1', ticketRef: 'FOO-1',
    createdAt: '2026-05-02T00:00:00Z',
    inputs: { problemStatement: 'X', flows: [{ name: 'A', screens: 1 }] },
  })
  const ao = manifest.nodes.find((n) => n.component === 'AnalysisOverview')
  assert.equal(ao.sections.length, 5)
  assert.equal(ao.sections.filter((s) => s.key === 'gap-check').length, 1)
})
