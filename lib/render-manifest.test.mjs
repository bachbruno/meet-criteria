...
test('mudanca: plugin data role propagada em todas as camadas (list/comparative/slots)', () => {
  const args = baseArgs('mudanca', { inputs: { problemStatement: 'x', pairs: [{ label: 'Tela 01' }] } })
  const m = buildRenderManifest(args)
  const cmp = m.nodes.find((n) => n.id === 'comparative')
  assert.equal(cmp.pluginData.role, 'comparative-list')
  assert.equal(cmp.children[0].pluginData.role, 'comparative')
  assert.equal(cmp.children[0].slots[0].pluginData.role, 'screen-slot')
  assert.equal(cmp.children[0].slots[0].pluginData.pairIndex, 0)
})

test('conceito: plugin data role propagada em todas as camadas', () => {
  const args = baseArgs('conceito', { inputs: { problemStatement: 'x', variants: ['A','B'], decisionCriteria: 'X' } })
  const m = buildRenderManifest(args)
  const variants = m.nodes.find((n) => n.id === 'variants')
  assert.equal(variants.pluginData.role, 'comparative-list')
  assert.equal(variants.pluginData.kind, 'variants')
  assert.equal(variants.children[0].pluginData.role, 'comparative')
  assert.equal(variants.children[0].slots[0].pluginData.role, 'screen-slot')
  assert.equal(variants.children[0].slots[0].pluginData.variantIndex, 0)
})

test('Comparative dispatch usa props.kind, não structureNode.id', () => {
  // Constrói um template artificial onde o id NÃO é 'variants' mas props.kind é,
  // pra garantir que dispatch usa kind corretamente.
  const fakeTemplate = {
    type: 'feature',
    version: '1.0.0',
    label: 'Fake',
    layout: { kind: 'horizontal-columns', gap: 80, padding: 64, background: '#000000' },
    structure: [
      { id: 'context', component: 'ContextMacro', required: true, props: { icon: 'star' } },
      { id: 'problem-statement', component: 'ProblemStatement', required: true },
      { id: 'side-by-side', component: 'Comparative', required: true, minCount: 2, maxCount: 3,
        props: { kind: 'variants', labels: ['Opção A', 'Opção B', 'Opção C'] } },
      { id: 'final-analysis', component: 'FinalAnalysis', required: true,
        sections: ['resolution', 'strengths', 'attention', 'discussion'] },
    ],
    checks: { deterministic: ['problem-statement-not-empty'] },
  }
  const identity = resolveIdentity({ mode: 'default' })
  const m = buildRenderManifest({
    ...
    ticketRef: 'X',
    createdAt: FIXED_DATE,
    inputs: { problemStatement: 'x', variants: ['A', 'B'] },
  })
  const node = m.nodes.find((n) => n.id === 'side-by-side')
  // Se dispatch usasse id, cairia no buildComparativeNode (que lê pairs) e falharia.
  // Como usa props.kind === 'variants', vai pro buildVariantsNode e produz children[0].slots
  assert.equal(node.children.length, 1)
  assert.equal(node.children[0].slots.length, 2)
  assert.equal(node.children[0].slots[0].label, 'Opção A')
})

test('Token resolution cobre props arbitrárias, não só tokenBackground/layout.background', () => {
  // Constrói template com {token:...} num prop arbitrário.
  const fakeTemplate = {
    type: 'feature',
    version: '1.0.0',
    label: 'Token-everywhere',
    layout: { kind: 'horizontal-columns', gap: 80, padding: 64, background: '{token:template.background}' },
    structure: [
      { id: 'context', component: 'ContextMacro', required: true,
        props: { icon: 'star', tokenBackground: '{token:tag.context.background}', customColor: '{token:anchor.dot.color}' } },
      { id: 'problem-statement', component: 'ProblemStatement', required: true },
  ...
  const ctx = m.nodes.find((n) => n.id === 'context')
  // tokenBackground e customColor devem virar hex via resolução automática
  assert.match(ctx.style.background, /^#[0-9a-f]{6}$/)
  assert.equal(ctx.style.background, DEFAULT_TOKENS['tag.context.background'])
})

test('mudanca: comparative-list carrega kind="before-after"', () => {
  const args = baseArgs('mudanca', { inputs: { problemStatement: 'x', pairs: [{ label: 'Tela 01' }] } })
  const m = buildRenderManifest(args)
  const cmp = m.nodes.find((n) => n.id === 'comparative')
  assert.equal(cmp.pluginData.role, 'comparative-list')
  assert.equal(cmp.pluginData.kind, 'before-after')
})

test('Token references nas props sempre resolvem para hex no manifest final (integration)', () => {
  // Sanity contract: nenhum {token:...} literal deve sobreviver no manifest.
  const m = buildRenderManifest(baseArgs('feature', {
    inputs: { problemStatement: 'x', flows: [{ name: 'F', screens: 1 }] },
  }))
  const stringified = JSON.stringify(m)
  assert.doesNotMatch(stringified, /\{token:[^}]+\}/, 'Manifest contém referência {token:...} não-resolvida')
  assert.match(m.layout.background, /^#[0-9a-f]{6}$/)
})
