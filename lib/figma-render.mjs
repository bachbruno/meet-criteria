...
function buildScreenNameTag(tag, tagPluginData) {
  ...
  setPluginData(f, tagPluginData)
  return f
}

// Paste-to-replace semantics: clone the user's screen and drop it directly at
// the placeholder coordinates inside the section. No iPhone wrapper frame is
// created — the cloned node IS the screen slot, and carries the screen-slot
// plugin data so /meet-criteria-anchor and friends can find it later.
async function cloneScreen(placeholder, figId, screenPluginData) {
  if (!figId) return null
  const original = await figma.getNodeByIdAsync(figId)
  if (!original || typeof original.clone !== 'function') return null
  const dup = original.clone()
  dup.x = placeholder.x
  dup.y = placeholder.y
  setPluginData(dup, screenPluginData)
  return dup
}
...
function buildScreenJustificationText(spec) {
  ...
  t.name = 'Screen justification — ' + (spec.pluginData && spec.pluginData.flowId != null ? spec.pluginData.flowId : '') + ' / ' + (spec.pluginData && spec.pluginData.screenIndex != null ? spec.pluginData.screenIndex : '')
  t.x = spec.x
  t.y = spec.y
