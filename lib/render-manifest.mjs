...
function buildFlowsNode(structureNode, inputs, layout) {
  ...
  // selectionNames is an optional flat array of screen names matching the
  // flattened slot order across flows. When provided, it populates each
  // slot tag's text; otherwise falls back to the generic "Screen name".
  const selectionNames = Array.isArray(inputs.selectionNames) ? inputs.selectionNames : null
  let slotCursor = 0
  ...
  const children = layout.flows.map((flowLayout, i) => {
    ...
    return {
      ...
      screens: flowLayout.screens.map((s) => {
        const provided = selectionNames ? selectionNames[slotCursor] : null
        slotCursor++
        const tagText = (typeof provided === 'string' && provided.trim() !== '')
          ? provided.trim()
          : 'Screen name'
        return {
...
function buildAnalysisOverviewNode(structureNode, inputs, layout) {
  const baseSections = structureNode.sections ?? ['resolution', 'validation', 'attention', 'discussion', 'gap-check']
  // Ensure gap-check is present even when older templates still list only 4 keys.
  // Task 5 will update templates/feature.jsonc to include gap-check explicitly.
  const sectionKeys = baseSections.includes('gap-check') ? baseSections : [...baseSections, 'gap-check']
  return {
    id: structureNode.id,
