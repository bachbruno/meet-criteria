...
function buildComparativeNode(structureNode, { pairs }) {
  ...
  if (list.length < minCount) {
    throw new RenderInputError(`mudanca.pairs tem ${list.length}; mínimo ${minCount} (minCount)`, { code: 'PAIRS_BELOW_MIN' })
  }
  if (list.length > maxCount) {
    throw new RenderInputError(`mudanca.pairs tem ${list.length}; máximo ${maxCount} (maxCount)`, { code: 'PAIRS_ABOVE_MAX' })
  }
